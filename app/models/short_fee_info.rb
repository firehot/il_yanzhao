  ...
  state_machine :initial => :draft do
    after_transition do |info,transition|
      info.short_fee_info_lines.each do |line|
        line.carrying_bill.from_write_off if info.org_id.eql?(line.carrying_bill.from_org_id)
        line.carrying_bill.to_write_off if info.org_id.eql?(line.carrying_bill.to_org_id)
      end
    end
    event :write_off do
