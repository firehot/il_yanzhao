class AddOrderToOrg < ActiveRecord::Migration
  def self.up
    ...
    sf.update_attributes(:default_action => "orgs_path('search[is_active_eq]' => 1 )")
    Notify.create(:notify_text => "tip: ctrl + b : 新建机打运单 ctrl + z 运单高级查询 n:新建 e:修改 s:保存 f:查询 d:删除 p:打印 x:导出")
  end
